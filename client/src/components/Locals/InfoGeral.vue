<template>
  <panel title="Informações Gerais">
    <div title="subheader">
      <h5>Estes são todos os locais para buscar Informações</h5>
    </div>
    <div v-for="local in locals" :key="local.idLOCAL">
      <v-layout>
        <v-flex xs10 offset-xs1 class="mt-2 mb-2">
          <div v-if="local.localImage != ''" class="imgInfo">
  ...
  watch: {
    '$route.query': {
      ...
      async handler (values) {
        this.locals = (await QuizService.indexInfoGeral(values)).data
      }
    }
  }
  // async mounted () {
